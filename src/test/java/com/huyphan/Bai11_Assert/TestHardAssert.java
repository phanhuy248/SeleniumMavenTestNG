...
public class TestHardAssert extends BaseTest {
    ...
    public void testHardAssert() throws InterruptedException {
        ...
        Assert.assertEquals(originalTitle, expectedTitle,"The tiile of the page is not correct");
        Thread.sleep(2000);
    }
    @Test(priority = 2)
    public void testLoginCRM() throws InterruptedException {
        System.out.println("Thực hiện đăng nhập CRM");
...
//        boolean checkMenuDashboard = driver.findElement(By.xpath(LocatorCRM.menuDashboard)).isDisplayed();
        Assert.assertTrue(!checkMenuDashboard.isEmpty(),"Login CRM failed");
        Thread.sleep(2000);
    }
    @Test(priority = 3)
    public void addNewCustomer() throws InterruptedException {
        ...
        Assert.assertFalse(checkMenuDashboard.isEmpty(), "Customer Page is not displayed");
        String headerCustomerSummary = driver.findElement(By.xpath(LocatorCRM.headerCustomerSummary)).getText();
        Assert.assertEquals(headerCustomerSummary, "Customers Summary1", "Header is not correct");
